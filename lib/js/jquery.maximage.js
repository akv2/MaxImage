...
(function($) {
	$.fn.maximage = function(settings) {
		...
		var config = {
			cssBackgroundSize: true,  // Force run the functionality used for old browsers
		...
		this.each(function(i) {
			...
			*/
			...
			var Cycle = {
				setup: function(){
					...
					var cycleOptions = $.extend(settings.cycleOptions, {
						...
						width:$.Window.data('w'),
						slideResize: false,
						easing: ($.Maximage.cssTransitions ? 'easeForCSSTransition' : 'swing')
					});
			...
			$.Maximage = Utils.browser_tests();
			
			// Begin setup
			if($.Maximage.cssBackgroundSize){
				Modern.setup();
			}else{
				Old.setup();
			}
