...
*/
/*!	
 * Maximage Version: 2.0.72 (08-Oct-2012) - http://www.aaronvanderzwan.com/maximage/2.0/
 */
		...
		this.each(function() {
			...
			*/
			...
			var Old = {
				...
				preload: function(){
					// Load the image (Rewritten for IE stack overflow bug)
					if($.Slides.length > 0){					
						for(var q in $.Slides) {
							var $img = $('<img />');
							
							// ImagesLoaded (using ImagesLoaded plugin included below)
							$img
								.data('mc-pos', q)
								.imagesLoaded(function() {
									var $loaded = $(this),
										pos = $loaded.data('mc-pos');
									
									// Add to DOM so we can get dimensions
									...
									// Add it to DOM
									$loaded.appendTo($('div.mc-image-n' + pos));
			...
			if(typeof settings == 'string'){
				var to, $storageEl = $self.is('object') ? $self.parents('video').first() : $self; // Can't assign .data() to '<object>'
